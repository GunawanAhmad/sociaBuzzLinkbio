...
(0, _previewHandler.previewHandler)();
      ...
      {"./circleLinkHandler.js":1,"./inputFormHandler.js":2,"./rearrangeHandler.js":3,"./previewHandler.js":4},
    ],1: [
      function (require, module, exports) {
        "use strict";
...
function circleLinkHandler() {
  ...
  cirlceLinkList.forEach(function (element, index) {
    element.addEventListener("click", function () {
      ...
      circleModalImg.src = selectedCirlceLink.querySelector("img").getAttribute("src");

      circleImg.src = circleModalImg.src;
      circleImg.index = index;

      circelModalTitle.value = selectedCirlceLink.querySelector(".cirlce-link-title").innerText;
  ...
  var tempImgSrc = "";
  document.getElementById("circle-link-img-input").addEventListener("change", function () {
  ...
  document.getElementById("circle-link-save-btn").addEventListener("click", function () {
    ...
    selectedCirlceLink.querySelector(".cirlce-link-title").innerText = circelModalTitle.value;
...
function getAllCircleValue() {
  cirlceLinkList.forEach(function (elm, idx) {
    var img = elm.querySelector("#circle-img-" + (idx + 1)).getAttribute("src");
    ...
    ArrOfCircleLinkObj.push(new CircleLinkObj(active, title, img, link, elm, idx));
  });
}
...
function CircleLinkObj(active, title, imgSrc, link, elm, index) {
  ...
  this.deleteBtn.addEventListener("click", function () {
    (0, _deleteHandler.deleteHandler)(_this.elm, "circle", _this, updateCircleLinkElm);
  });
}
...
function updateCircleLinkElm(index) {
  ...
  cirlceLinkList[index].querySelector("#circle-img-" + (index + 1)).setAttribute("src", "");
...
var tambahFiturSpesialBtn = document.querySelectorAll(".add-fitur-spesial-btn");
var arrOfInputLinks = [];
...
function reloadFlexSelect() {
  $(document).ready(function () {
    $("select.flexselect").flexselect({
      allowMismatch: true,
      inputNameTransform: function inputNameTransform(name) {
        return "new_" + name;
      }
    });
  });
...
function inputFormHandler() {
  ...
  function addNewInput(type) {
    ...
    switch (type) {
      case "regular":
        HTMLString = (0, _inputFormHTMLSource.regularInputLink)(regularInputLinkCount);
        regularInputLinkCount++;
        break;
      case "header":
        HTMLString = (0, _inputFormHTMLSource.headerInputLink)(headerInputLinkCount);
        headerInputLinkCount++;
        break;
      case "donasi":
        HTMLString = (0, _inputFormHTMLSource.donasiInputLink)(donasiInputLinkCount);
        donasiInputLinkCount++;
        break;
      case "digital-akses":
        HTMLString = (0, _inputFormHTMLSource.digitalAksesInputLink)(digitalAksesInputLinkCount);
        digitalAksesInputLinkCount++;
        reloadFlexSelect();
        break;
      case "konten":
        HTMLString = (0, _inputFormHTMLSource.kontenInputLink)(kontenInputLinkCount);
        kontenInputLinkCount++;
        reloadFlexSelect();
        break;
      case "paket-jasa":
        HTMLString = (0, _inputFormHTMLSource.paketJasaInputLink)(paketJasaInputLinkCount);
        paketJasaInputLinkCount++;
        reloadFlexSelect();
        break;
      case "kerjasama":
        HTMLString = (0, _inputFormHTMLSource.kerjasamaInputLink)(kerjasamaInputLinkCount);
        kerjasamaInputLinkCount++;
        reloadFlexSelect();
        break;
      case "embed":
        HTMLString = (0, _inputFormHTMLSource.embedInputLink)(embedInputLinkCount);
        embedInputLinkCount++;
        break;
      case "whatsapp":
        HTMLString = (0, _inputFormHTMLSource.whatsappInputLink)(whatsappInputLinkCount);
        whatsappInputLinkCount++;
        break;
      case "email":
        HTMLString = (0, _inputFormHTMLSource.emailInputLink)(emailInputLinkCount);
        emailInputLinkCount++;
        break;
      case "email-collection":
        HTMLString = (0, _inputFormHTMLSource.emailColletionInputLink)(emailColletionInputLinkCount);
        emailColletionInputLinkCount++;
        break;
      case "video-request":
        HTMLString = (0, _inputFormHTMLSource.videoRequestInputLink)(videoRequestInputLinkCount);
...
function rearrangeHandler() {
  ...
  //for circle link
  var circleLinkContainer = document.querySelector(".rounded-link-wrapper");
